...
from keys import client_items_inlines, pricelist_inlines
...
async def pricelist_command(message: Message) -> None:
    ...
    if len(names) > 6:
        ...
        first_page = False
    await message.answer('Прайс-лист:', reply_markup=pricelist_inlines(names[:last_num], last_num, first_page)) 
